...
class Virtual_Angle:
    ...
    def PlaceVO(self,u_score,v_score):
        # return True
        # return u_score<=120 and v_score<=120
        if self.fail_incre > 100:
            return u_score<=100 and v_score<=100
        elif self.fail_incre > 200:
        #     return u_score<=100 and v_score<=100
        # elif self.fail_incre > 400:
        #     return u_score<=130 and v_score<=130
        # elif self.fail_incre > 500:
        #     return u_score<=180 and v_score<=180
        # elif self.fail_incre > 600:
        #     return u_score<=250 and v_score<=250
        # elif self.fail_incre > 700:
        #     return u_score<=300 and v_score<=300
        # elif self.fail_incre > 800:
            return True
    ...
    def scene_check(self,data):
        ...
        if(self.PlaceVO(select_u_score,select_v_score)):
            ...
            print(f"score({u_score:.0f}, {v_score:.0f}), select angle({select_angle:.0f})")
            ...
            self.fail_incre += 1
            print(f"[virtual_angle/scene_check] score({u_score:.0f}, {v_score:.0f}) too high, select angle({select_angle:.0f})", end='\r',flush=True)
            write2file("/home/brian/catkin_ws/src/IRL_SLAM/log.csv", f"{u_score:.0f}, {v_score:.0f}\n")
            return []
        # return False
